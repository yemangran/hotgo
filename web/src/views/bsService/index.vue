<template>
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="服务项目">
    ...
    <n-card :bordered="false" class="proCard">
      ...
      <BasicTable  ref="actionRef"  :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000" :cascade="false" :expanded-row-keys="expandedKeys" @update:expanded-row-keys="updateExpandedKeys"  >
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/bsService/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
...
<script lang="ts" setup>
  ...
  import { List, Delete } from '@/api/bsService';
  import { PlusOutlined, AlignLeftOutlined } from '@vicons/antd';
  import { columns, schemas, newState } from './model';
