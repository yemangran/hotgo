...
type (
	...
	ISysBsService interface {
		...
		Model(ctx context.Context, option ...*handler.Option) *gdb.Model
		// List 获取服务项目列表
		List(ctx context.Context, in *sysin.BsServiceListInp) (list []*sysin.BsServiceListModel, totalCount int, err error)
		// Edit 修改/新增服务项目
		Edit(ctx context.Context, in *sysin.BsServiceEditInp) (err error)
		// Delete 删除服务项目
		Delete(ctx context.Context, in *sysin.BsServiceDeleteInp) (err error)
		// View 获取服务项目指定信息
